...
void LSM9DS1::I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
    ...
    buffer[0] = subAddress;              /* 1バイト目にレジスタアドレスをセット. */
    memcpy(&buffer[1], &data, length);  /* 2バイト目以降にデータをセット. */

    /* I2C-Writeメッセージを作成する. */
    struct i2c_msg message = { address, 0, static_cast<uint16_t>(length + 1), buffer };
    struct i2c_rdwr_ioctl_data ioctl_data = { &message, 1 };

    /* I2C-Writeを行う. */
    if (ioctl(_fd, I2C_RDWR, &ioctl_data) != 1) {
      fprintf(stderr, "i2c_write: failed to ioctl\n");
      free(buffer);
      close(_fd);
      exit(EXIT_FAILURE);
    }
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
    ...
    struct i2c_msg messages[] = {          
        { address, 0, 1, &subAddress },         /* レジスタアドレスをセット. */
        { address, I2C_M_RD, 1, &data },  /* dataにlengthバイト読み込む. */
    };
    struct i2c_rdwr_ioctl_data ioctl_data = { messages, 2 };

    /* I2C-Readを行う. */
    if (ioctl(_fd, I2C_RDWR, &ioctl_data) != 2) {
        fprintf(stderr, "i2c_read: failed to ioctl\n");
        close(_fd);
        exit(EXIT_FAILURE);
    }
...
uint8_t LSM9DS1::I2CreadBytes(uint8_t address, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
    ...
    struct i2c_msg messages[] = {          
        { address, 0, 1, &subAddress },         /* レジスタアドレスをセット. */
        { address, I2C_M_RD, count, temp_dest },  /* dataにlengthバイト読み込む. */
    };
    struct i2c_rdwr_ioctl_data ioctl_data = { messages, 2 };
